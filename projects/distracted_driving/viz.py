...
class Vizualizer:
    def __init__(self, config, face_config, pose_config, tqdm=None):
        ...
        Path(f'{self.config.OUTPUT_FOLDER}/report_plots').mkdir(parents=True, exist_ok=True)
    ...
    def plot_samples(self, num=5, out_file='class_samples.jpg'):
        ...
        fig, axes = plt.subplots(10, num, figsize=[1100*px, 2200*px])
        pad = 5 # in points
        classes = list(self.config.class_dict.keys())
        ...
        for row, class_code in enumerate(classes):
            ...
            for i, img_file in enumerate(imgs):
                ...
                axis = axes[row][i]
                if i == 0:
                    axis.annotate(class_name, xy=(0.35, 1), xytext=(0, pad),
                        xycoords='axes fraction', textcoords='offset points',
                        fontsize=12, ha='center', va='baseline')
                axis.imshow(img)
                axis.set_xticks([])
                axis.set_yticks([])
    ...
    def plot_faces_summary(self, out_file='faces_distribution.jpg'):
        ...
        pd_merged.plot(title='Face Identification Summary', figsize=(6, 4), kind='bar', 
    ...
    def display_faces(self, per_group_count=3, out_file='faces.jpg'):
        ...
        fig, axes = plt.subplots(per_group_count, 3, figsize=[12, 9], dpi=72)
        ...
        for i in range(per_group_count):
            for j, image in enumerate(sampled[i]):
                ...
                axes[i][j].axis('off')
        plt.suptitle('Random Samples Annotated With Faces Identified', fontsize = 20)
        plt.tight_layout()
        plt.savefig(f'{self.config.OUTPUT_FOLDER}/report_plots/{out_file}', dpi=300)    
        plt.show()
    ...
    def display_poses(self, rows=3, out_file='poses.jpg'):
        ...
        dataloader = DataLoader(dataset, num_workers=0, batch_size=1,
                                shuffle=True, collate_fn=dataset.get_image_from)
        fig, axes = plt.subplots(rows, 3, figsize=[18, 16], dpi=72)
        axes[0][0].set_title('Original', fontsize = 16)
        axes[0][1].set_title('Annotated With Pose', fontsize = 16)
        axes[0][2].set_title('Pose Extracted', fontsize = 16)
        ...
        for i, sample in enumerate(dataloader):
            ...
            axes[i][2].axis('off')
        plt.suptitle('Random Samples Annotated With Poses', fontsize = 21)
        plt.tight_layout()
        plt.savefig(f'{self.config.OUTPUT_FOLDER}/report_plots/{out_file}', dpi=300)
        plt.show()
    ...
    def plot_keypoints_relative_positions(self, out_file='wrists_offset_distribution.jpg'):
        ...
        col = 0
        ax = axes[row][col]
        ax.set_title('Right Wrist: Relative x coord', fontsize=18)
        sns.boxplot(ax=ax, y = df['right_wrist_offset_x'], x = df['label_name'])

        row = 0
        col = 1
        ax = axes[row][col]
        ax.set_title('Right Wrist: Relative y coord', fontsize=18)
        sns.boxplot(ax=ax, y = df['right_wrist_offset_y'], x = df['label_name'])

        row = 1
        col = 0
        ax = axes[row][col]
        ax.set_title('Left Wrist: Relative x coord', fontsize=18)
        sns.boxplot(ax=ax, y = df['left_wrist_offset_x'], x = df['label_name'])

        row = 1
        col = 1
        ax = axes[row][col]
        ax.set_title('Left Wrist: Relative y coord', fontsize=18)
        sns.boxplot(ax=ax, y = df['left_wrist_offset_y'], x = df['label_name'])

        for ax in axes.flatten():
            ax.tick_params(axis='x', labelrotation = 30, labelright=True, labelsize=14)
            # ax.set_xticklabels(ax.get_xticklabels(), rotation=30, ha='right')
            ax.set_xlabel('')
            ax.set_ylabel('')

        plt.suptitle('Relative Positions of Wrists', fontsize=24)
        plt.tight_layout()
        plt.savefig(f'{self.config.OUTPUT_FOLDER}/report_plots/{out_file}', dpi=300)
        plt.show()

    def plot_hand_angles(self, out_file='hand_angles_distribution.jpg'):
        feature_extractor = feature_helpers.FeatureExtractor(self.config, self.face_config, self.pose_config, self.tqdm)
        df = feature_extractor.all_hand_angles(sample_type=enums.SampleType.TRAIN_VALIDATION)
        df['label_name'] = df['label'].map(self.config.class_dict)
        fig, axes = plt.subplots(nrows=1, ncols=2, figsize=[18, 12], dpi=72)
            
        ax = axes[0]
        ax.set_title('Left Hand Angle', fontsize=18)
        sns.boxplot(ax=ax, y = df['lh_angle'], x = df['label_name'])

        ax = axes[1]
        ax.set_title('Right Hand Angle', fontsize=18)
        sns.boxplot(ax=ax, y = df['rh_angle'], x = df['label_name'])
        ...
        plt.suptitle('Hand Angles', fontsize=24)
        plt.tight_layout()
        plt.savefig(f'{self.config.OUTPUT_FOLDER}/report_plots/{out_file}', dpi=300)
        plt.show()
    ...
    def plot_body_parts_feature_distribution(self, features_csv, out_file):
        ...
        sns.boxplot(list_left_arm_orients, ax=ax)
        ax.set_xticklabels(class_tags.tolist())
        ax.set(xlabel='Class', ylabel='Orientation (in rad)', title='Left arm orientations across different classes')
        ax.tick_params(axis='x', labelrotation = 60, labelright=True, labelsize=14)
        ...
        for this_tag in class_tags:
            right_arm_orients = df_without_na.loc[df['label_name'] == this_tag]['RightOrient'].tolist()
            list_right_arm_orients.append(right_arm_orients)
        # Plot box-plot for right arm
        ax = axes[1]
        sns.boxplot(list_right_arm_orients, ax=ax)
        ax.set_xticklabels(class_tags.tolist())
        ax.set(xlabel='Class', ylabel='Orientation (in rad)', title='Right arm orientations across different classes')
        ax.tick_params(axis='x', labelrotation = 60, labelright=True, labelsize=14)
        if out_file is not None:
            plt.savefig(f'{self.config.OUTPUT_FOLDER}/report_plots/{out_file}', dpi=300)
        plt.show()