...
class FeatureExtractor:
    ...
    def all_hand_angles(self, sample_type=enums.SampleType.TRAIN_VALIDATION):
        data = self.load_data(image_types=None, shuffle=False, 
            sample_type=sample_type, include_feature_vectors=False)
        [rh_angles, lh_angles] = self.load_feature_vectors(
            self.config.FEATURE_VECTORS_FOLDER, 
            data[enums.DataColumn.FILENAME.value], 
            data[enums.DataColumn.LABEL.value],
            features=set())
        df = pd.DataFrame(zip(data[enums.DataColumn.LABEL.value], rh_angles, lh_angles), 
            columns=['label', 'rh_angle', 'lh_angle'])
        df['rh_angle'] = df['rh_angle'] / 360
        df['lh_angle'] = df['lh_angle'] / 360
        return pd.DataFrame(zip(data[enums.DataColumn.LABEL.value], rh_angles, lh_angles), 
            columns=['label', 'rh_angle', 'lh_angle'])
